...
export default function AppShellHeader() {
  ...
  const [userEmail, setUserEmail] = useState("workspace@issuefixu.com");
  ...
  useEffect(() => {
    ...
    getCurrentUser()
      .then((user) => {
        ...
        setUserEmail(user.email || "workspace@issuefixu.com");
      })
      .catch(() => {
        if (!mounted) return;
        setUserName("Team Member");
        setUserEmail("workspace@issuefixu.com");
      });
  ...
  return (
    <header className="rounded-2xl bg-(--surface)/95 px-3 py-3 shadow-card sm:px-4">
